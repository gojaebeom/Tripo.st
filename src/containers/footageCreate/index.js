...
export default function FootageCreate (props) {
    ...
    const onSubmit = async ( ) => {
        //유효성 검사
        let result = validationInput(thumbnail.origin, (message="썸네일 이미지를 등록해주세요.") => {
        ...
        if(!result) return;
        result = validationInput(footage.file, (message="영상을 등록해주세요.") => {
            if(props.lang == 1) message = 'Please register the footage.';
            if(props.lang == 2) message = 'Hãy đăng ký footage.';
            props.setSnackbar({type:'WARNING', open:true, message: message, time : 2000,});
        });
        if(!result) return;
        ...
        const res = await create('/api/footages/create/', formData);
    ...
    if(redirect){
        return <Redirect to={`/tripo-clips`} />
    }
    ...
    return (
        <div className="tripo-clip-container">
            ...
            <div className="tripo-clip-create-wrap">
                ...
                <h2>
                    { props.lang == 0 && '여행클립 생성'}
                    { props.lang == 1 && 'Create TripClip'}
                    { props.lang == 2 && 'Tạo clip du lịch'}
                </h2>
